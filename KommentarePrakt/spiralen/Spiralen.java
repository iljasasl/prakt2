...
public class Spiralen extends JPanel {
  ...
  private void drawSpiral(Graphics graphics, String[] args){
    ...
    /**
     * AUFGABE: Beiargs[0], usw. handelt es sich um Strings. Sie können einen int-Wert aus einem String mit der Methode Integer.parseInt und einen double-Wert mit Double.parseDouble auslesen.
     * 
     * Einlesen v parameter aus Kommandozeilenargumenten anhand von try und catch
     */
    try {
      if(args.length == 3){ 
      ...
      /**
       * Wenn nicht passende Anzahl an args => Fehlermeldung
       */
      else {
     ...
     * Überlegen Sie sich, welche ungültigen Werte der Nutzer übergeben kann:
      ▶ Waspassiert, wenn Sie nicht alle drei Kommandozeilenargumente beim Aufruf angeben? Wie
      können Sie darauf reagieren? => if(args.length ==3) SONST STANDARDWERT
      ▶ Waspassiert, wenn Sie statt einer Zahl ein Wort übergeben? Wie können Sie darauf reagieren? => try{...}catch(e){STANDARDWERTE}
      ▶ Wassind die gültigen Werte für die Eingabeparameter? => 1. Positive Iteration 2. Verkürzungsfaktor zwischen 0 und 1 (mehr unten)
      Implementieren Sie zumindest eine Prüfung auf Gültigkeit!
     */

     /**
      * 1. GÜLTIGKEITSPRÜFUNG: Positive Iteration
      */
    if (iterations <= 0) {
    ...
    /**
     * 2. GÜLTIGKEITSPRÜFUNG: Verkürzungsfaktor zwischen 0 und 1
     */
